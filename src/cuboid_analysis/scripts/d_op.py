...
csv_path = os.path.join(home_dir, '.ros', 'output.csv')

# 辺の期待値と分散共分散行列の定義
mean, covariance_matrix = read_output_csv(csv_path)
...
# Move_robotで読み取るために、D-optimalityの値をファイルに書き出す(亀山追加)
output_file = os.path.join(home_dir, '.ros', 'd_optimal_value.txt')
with open(output_file, 'w') as f:
    f.write(f"{d_optimal_value}")
...
try:
    ...
    # Move_robotで読み取るために、D-optimalityの値をファイルに書き出す
    output_file = os.path.join(home_dir, '.ros', 'd_optimal_value.txt')
    with open(output_file, 'w') as f:
        f.write(f"{d_optimal_value}")
